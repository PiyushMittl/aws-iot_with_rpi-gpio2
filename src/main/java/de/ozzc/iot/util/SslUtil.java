...
/**
 * SSLUtil contains a function to create a SSLSocketFactory to be used by the
 * Mqtt Client for authentication.
 *
...
public class SslUtil {
	...
	/**
	 * Creates a custom SSLSocketFactory which can be used to authenticate to
	 * AWS IoT with Eclipse Paho.
	 *
	 * @param rootCaCertFile
	 *            The file path of the root CA file.
	 * @param clientCertFile
	 *            The file path of the client cert file.
	 * @param privateKeyFile
	 *            The file path of the client private key.
	 * @return A SSLSocketFactory that validates the server against the root CA
	 *         file and the client on the server with the client cert and
	 *         private key.
	 * @throws Exception
	 */
	public static SSLSocketFactory getSocketFactory(final String rootCaCertFile, final String clientCertFile,
		...
		System.out.println("created rootCaCert X509Certificate");
		
		X509Certificate clientCaCert = getX509Certificate(clientCertFile);
		System.out.println("created clientCaCert X509Certificate");
		
		
		PEMKeyPair clientKeyPair;
		try (PEMParser clientPrivateKeyParser = new PEMParser(new FileReader(privateKeyFile))) {
		...
		// client key and certificates are sent to server for client
	...
	private static X509Certificate getX509Certificate(final String certFileName)
		...
		//StringReader sr=new StringReader("ww");
		
		
		
		try (PEMParser certParser = new PEMParser(new FileReader(certFileName))) {
			return new JcaX509CertificateConverter().setProvider(BC_PROVIDER_ID)
...
//		try (PEMParser certParser = new PEMParser(new StringReader(certFileName))) {
//			return new JcaX509CertificateConverter().setProvider(BC_PROVIDER_ID)
//					.getCertificate((X509CertificateHolder) certParser.readObject());
//		} catch (IOException | CertificateException e) {
//			LOGGER.error("Failed to load certificate file at {} with error: {}", certFileName, e.getMessage());
